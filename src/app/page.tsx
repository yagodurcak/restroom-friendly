...
import { Search, Map, List, X, MapPin, Store } from "lucide-react";
...
export default function Home() {
  ...
  return (
    /* Desktop: centered app frame on warm background. Mobile: full screen */
    <div
      className="min-h-screen flex items-start md:items-center justify-center"
      style={{ background: "#E8E3DC" }}
    >
      <div
        className="relative flex flex-col overflow-hidden w-full md:rounded-3xl md:shadow-2xl"
        style={{
          height: "100dvh",
          maxWidth: 430,
          background: "#F5F3EE",
      ...
      >
          ...
          <div className="flex gap-2 mt-3 overflow-x-auto pb-0.5" style={{ scrollbarWidth: "none" }}>
            {FILTER_OPTIONS.map((f) => (
              <button
                ...
                style={{
                  background: activeFilter === f ? "#12907A" : "#fff",
                  color: activeFilter === f ? "#fff" : "#6E6E82",
                  border: `1px solid ${activeFilter === f ? "#12907A" : "rgba(28,28,40,0.1)"}`,
                  fontSize: 12,
                  fontWeight: 600,
                }}
              >
          ...
          <div className="flex rounded-xl overflow-hidden" style={{ border: "1px solid rgba(28,28,40,0.1)" }}>
            {(["map", "list"] as const).map((v) => (
              <button
                ...
                style={{
                  background: view === v ? "#12907A" : "#fff",
                  color: view === v ? "#fff" : "#6E6E82",
                  fontSize: 12,
                  fontWeight: 600,
                }}
              >
            ...
            <div className="h-full overflow-y-auto px-4 py-1 space-y-2" style={{ scrollbarWidth: "none" }}>
              {filteredLocations.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64 text-center">
                  ...
                  <p style={{ fontFamily: "var(--font-nunito), Nunito, sans-serif", fontWeight: 700, fontSize: 16, color: "#1C1C28" }}>
                ...
                filteredLocations.map((loc) => (
                  <LocationCard key={loc.id} location={loc} onClick={() => handleCardClick(loc.id)} />
                ))
              )}
              <div style={{ height: 24 }} />
            </div>
        ...
        {/* Join CTA (list view) */}
        {view === "list" && !showDetail && !showJoin && (
          <button
            onClick={() => setShowJoin(true)}
            className="mx-4 mb-4 mt-1 p-4 rounded-2xl flex items-center justify-between flex-shrink-0"
        ...
        {showDetail && selectedLocation && (
          <>
            <div
              ...
              style={{ background: "rgba(28,28,40,0.4)", zIndex: 40, backdropFilter: "blur(2px)" }}
            ...
            <div
              ...
              style={{
                ...
                zIndex: 50,
                maxHeight: "82%",
                boxShadow: "0 -8px 40px rgba(28,28,40,0.18)",
                display: "flex",
